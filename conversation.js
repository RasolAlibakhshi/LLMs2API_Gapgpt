...
export function createConversation(page, { timeout = 300000, pollInterval = 100, onBeforeSend, onProgress } = {}) {
  ...
  async function send(prompt, model, onChunk) {
    ...
    if (page.isClosed()) throw new ApiError('browser_unavailable', 'The conversation tab was closed.', 503, { retryable: true });
    try {
      const input = page.locator('.composer-textarea textarea, textarea.bidi-textarea').first();
      await input.waitFor({ state: 'visible', timeout: Math.min(timeout, 60000) }).catch(error => {
        if (page.isClosed()) throw error;
        if (/login|sign-?in|auth/i.test(page.url?.() || '')) {
          throw new ApiError('login_required', 'Sign in to GapGPT in the server browser.', 503);
        }
        throw new ApiError('composer_unavailable', 'GapGPT composer unavailable. Check account login and site dialogs.', 503);
      });
      ...
      await onBeforeSend?.();
      // From this point a click can have reached the site even if Playwright fails.
      delivery = 'unknown';
      ...
      while (Date.now() - started < timeout) {
        ...
        if (state.complete) {
          ...
          if (onChunk && state.text !== emitted) throw new Error('GapGPT revised already streamed text. Check the chat before retrying.');
          return state.text;
        }
      ...
      const normalized = asApiError(error, delivery);
      if (delivery === 'unknown' && normalized.delivery === 'not_sent') {
        normalized.delivery = 'unknown';
